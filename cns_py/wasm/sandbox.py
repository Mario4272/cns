    ...
    def execute(self, wasm_bytes: bytes, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # Wasmtime raises Trap for any WASI exit(1), but exit(0) might also look like exception?
                # Usually exit(0) is silent or specific ExitTrap.
                # simpler to assume if it raises, it failed, unless we inspect Exception type.
            ...
            if error:
                 # If we have output (e.g. partial write before crash? or exit(0) viewed as crash?)
                 # If output is valid JSON, maybe it was a success?
                 # Wasmtime 18+ might raise ExitTrap even for 0.
                 # Let's see if output is valid.
                 pass
        ...
        try:
            return json.loads(output_bytes.decode("utf-8"))
             ...
             raise RuntimeError(f"WASM returned invalid JSON: {output_bytes.decode('utf-8', errors='replace')}") from e

# Default instance (unlimited by default)
_SANDBOX = WasmSandbox()

def execute_rule(wasm_bytes: bytes, input_data: Dict[str, Any], sandbox_instance: Optional[WasmSandbox] = None) -> Dict[str, Any]:
