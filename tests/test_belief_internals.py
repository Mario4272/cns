...
from cns_py.cql.belief import _recency_modifier, _sigmoid
...
def test_recency_term_none_and_half_life_behavior():
    ...
    assert _recency_modifier(None, None, 365.0) == 0.0

    now = datetime.now(timezone.utc)
    # At t=0 days, recency should be ~1.0
    r0 = _recency_modifier(now, now, 10.0)
    assert 0.99 <= r0 <= 1.0

    # At half-life days, recency ~0.5
    r_half = _recency_modifier(now - timedelta(days=10.0), now, 10.0)
    assert 0.45 <= r_half <= 0.55

    # Very large delta drives toward 0.0 (but clamped ≥0)
    r_old = _recency_modifier(now - timedelta(days=10000), now, 10.0)
