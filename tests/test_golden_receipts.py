...
def test_explain_schema_stability():
    ...
    actual_json = json.loads(json.dumps(actual, default=lambda x: x.isoformat() if isinstance(x, datetime) else str(x)))
    ...
    assert actual_json == expected, "Current output does not match Golden Master. If change is intentional, run scripts/generate_golden.py."
...
def test_belief_term_stability():
    ...
    expected_factors = {"evidence", "contradictions", "recency", "source_rep"}
    assert set(terms.keys()) == expected_factors or set(terms.keys()).issuperset({"evidence", "recency"}), \
