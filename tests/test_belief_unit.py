...
def test_belief_compute_baseline_only():
    ...
    assert details["input_belief"] == 0.5
...
def test_belief_recency_recent_higher_than_old():
    ...
    conf_recent, d_recent = compute(0.5, now, cfg=cfg)
    conf_old, d_old = compute(0.5, now - timedelta(days=3650), cfg=cfg)
    assert d_recent["decay_factor"] > d_old["decay_factor"]
