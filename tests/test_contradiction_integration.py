...
from cns_py.cql.contradict import detect_fiber_contradictions, detect_all_contradictions
...
def seed_contradiction_scenario():
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("DELETE FROM atoms WHERE label LIKE 'TestFramework%' OR label LIKE 'TestTLS%'")
            ...
            cur.execute(
                ...
                (fiber1_id, past, future, 0.90, json.dumps({
            ...
            cur.execute(
                """INSERT INTO aspects(subject_kind, subject_id, valid_from, valid_to, belief, provenance)
                   VALUES ('fiber', %s, %s, %s, %s, %s)""",
                (fiber2_id, overlap_start, overlap_end, 0.95, json.dumps({
...
def test_contradiction_with_asof_resolves(seed_contradiction_scenario):
    ...
    query = f'MATCH label="TestFrameworkY" PREDICATE supports_tls ASOF {past_date} RETURN PROVENANCE'
    result = cql(query)
