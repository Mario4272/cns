...
def test_provenance_verify_happy_path():
    ...
    # 3. Call API
    resp = client.post(
        "/provenance/verify", json={"payload": payload, "signature": sig, "public_key": pub_hex}
    )
...
def test_provenance_verify_tampered():
    ...
    resp = client.post(
        "/provenance/verify",
        json={"payload": tampered_payload, "signature": sig, "public_key": pub_hex},
    )
...
def test_provenance_verify_invalid_key_format():
    ...
    resp = client.post(
        "/provenance/verify",
        json={"payload": {}, "signature": "00" * 64, "public_key": "not-a-hex-string"},
    )
    assert resp.status_code == 200
    data = resp.json()
    assert data["valid"] is False
