...
def test_c1_latest_tie_break(clean_truth_policy_data):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            # Create Source
            cur.execute("INSERT INTO atoms(kind, label) VALUES ('Entity', %s) RETURNING id", (f"{prefix}_S",))
            s_id = cur.fetchone()[0]
            
            # Create Dests
            cur.execute("INSERT INTO atoms(kind, label) VALUES ('Entity', %s) RETURNING id", (f"{prefix}_D1",))
            d1_id = cur.fetchone()[0]
            cur.execute("INSERT INTO atoms(kind, label) VALUES ('Entity', %s) RETURNING id", (f"{prefix}_D2",))
            d2_id = cur.fetchone()[0]
            
            # Create Fibers
            # We insert D1 first, D2 second so D1 usually has lower ID.
            cur.execute("INSERT INTO fibers(src, dst, predicate) VALUES (%s, %s, 'rel') RETURNING id", (s_id, d1_id))
            f1_id = cur.fetchone()[0]
            
            cur.execute("INSERT INTO fibers(src, dst, predicate) VALUES (%s, %s, 'rel') RETURNING id", (s_id, d2_id))
            ...
            cur.execute(
                "INSERT INTO aspects(subject_kind, subject_id, belief, observed_at, provenance) VALUES ('fiber', %s, 0.5, %s, %s)",
                (f1_id, t_old, prov)
            )
            cur.execute(
                "INSERT INTO aspects(subject_kind, subject_id, belief, observed_at, provenance) VALUES ('fiber', %s, 0.5, %s, %s)",
...
def test_c1_tie_break_exact_timestamp(clean_truth_policy_data):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("INSERT INTO atoms(kind, label) VALUES ('Entity', %s) RETURNING id", (f"{prefix}_S_Tie",))
            s_id = cur.fetchone()[0]
...
def test_c2_null_confidence(clean_truth_policy_data):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("INSERT INTO atoms(kind, label) VALUES ('Entity', %s) RETURNING id", (f"{prefix}_S_Null",))
            s_id = cur.fetchone()[0]
            
            # Atoms for dests
            d_ids = []
            for k in range(3):
                cur.execute("INSERT INTO atoms(kind, label) VALUES ('Entity', %s) RETURNING id", (f"{prefix}_D{k}",))
            ...
            f_ids = []
            for did in d_ids:
                cur.execute("INSERT INTO fibers(src, dst, predicate) VALUES (%s, %s, 'rel') RETURNING id", (s_id, did))
            ...
            # 1. Positive 0.2
            cur.execute("INSERT INTO aspects(subject_kind, subject_id, belief, observed_at, provenance) VALUES ('fiber', %s, 0.2, %s, %s)", (f_pos, t_fixed, prov))
            # 2. Zero 0.0
            cur.execute("INSERT INTO aspects(subject_kind, subject_id, belief, observed_at, provenance) VALUES ('fiber', %s, 0.0, %s, %s)", (f_zero, t_fixed, prov))
            # 3. NULL
            cur.execute("INSERT INTO aspects(subject_kind, subject_id, belief, observed_at, provenance) VALUES ('fiber', %s, NULL, %s, %s)", (f_null, t_fixed, prov))
...
def test_c3_contradiction_stability(clean_truth_policy_data):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("INSERT INTO atoms(kind, label) VALUES ('Entity', %s) RETURNING id", (f"{prefix}_S_Con",))
            s_id = cur.fetchone()[0]
            
            cur.execute("INSERT INTO atoms(kind, label) VALUES ('Entity', %s) RETURNING id", (f"{prefix}_Obj1",))
            o1_id = cur.fetchone()[0]
            cur.execute("INSERT INTO atoms(kind, label) VALUES ('Entity', %s) RETURNING id", (f"{prefix}_Obj2",))
            o2_id = cur.fetchone()[0]
            
            # Conflicting predicate 'is_valid'
            cur.execute("INSERT INTO fibers(src, dst, predicate) VALUES (%s, %s, 'is_valid') RETURNING id", (s_id, o1_id))
            f1_id = cur.fetchone()[0]
            cur.execute("INSERT INTO fibers(src, dst, predicate) VALUES (%s, %s, 'is_valid') RETURNING id", (s_id, o2_id))
            f2_id = cur.fetchone()[0]
            ...
            cur.execute(
                """INSERT INTO aspects(subject_kind, subject_id, valid_from, valid_to, belief, observed_at, provenance) 
